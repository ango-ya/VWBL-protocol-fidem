    ...
    ERC1155BurnableUpgradeable
{
     ...
     * We have 7 VWBLFidemToken-specific variables, so reserve 43 slots to make 50 total.
    ...
    function mint(
        ...
        _processMint(orderId, tokenId, customer, saleAmount, currency, paymentInvoiceId, vwblFee, getGatewayAddress());
     ...
     * @param orderIds Array of order IDs (generated by server)
    ...
    function mintBatch(
        ...
        require(orderIds.length == batchSize, "OrderIds length mismatch");
        ...
        require(currencies.length == batchSize, "Currencies length mismatch");
    ...
    function _processMint(
        ...
        // Store order IDs
        tokenIdToOrders[tokenId].push(orderId);
        ...
        // Emit events
        emit TokenMinted(orderId, tokenId, customer, saleAmount, currency, paymentInvoiceId);
        emit OrderCreated(orderId, tokenId, customer);
    }
    ...
    /**
     * @notice Get order count for a token
     */
    function getOrderCountByToken(uint256 tokenId) public view returns (uint256) {
        return tokenIdToOrders[tokenId].length;
    }

    /**
     * @notice Get order count for a customer
     */
    function getOrderCountByCustomer(address customer) public view returns (uint256) {
        return customerToOrders[customer].length;
    }

    /**
     * @notice Get orders for a token with pagination
     */
    function getOrdersByTokenPaginated(
        uint256 tokenId,
        uint256 offset,
        uint256 limit
        ...
        require(offset < orderIds.length, "Offset out of bounds");

        uint256 end = offset + limit > orderIds.length ? orderIds.length : offset + limit;

        MintReceipt[] memory result = new MintReceipt[](end - offset);
        for (uint256 i = offset; i < end; i++) {
            result[i - offset] = orders[orderIds[i]];
        }
        return result;
    }
}
